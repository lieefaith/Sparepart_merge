...
@section('content')
    <div class="container py-5 px-6">
                            ...
                            <tr class="ticket-row hover:bg-gray-50 cursor-pointer transition-colors">
                                ...
                                <td class="px-4 py-3 text-sm">
                                    <div class="flex items-center space-x-2">
                                        ...
                                        @else
                                            <span
                                                class="inline-flex items-center px-2 py-1 text-xs font-medium bg-yellow-100 text-yellow-800 rounded-full">Pending</span>
                                        @endif
                    ...
                    <div class="p-6">
                        ...
                        <div id="modal-content" style="display: none;">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4 text-sm">
                                <div><strong>Nama Tiket:</strong> <span id="modal-ticket-name"></span></div>
                                <div><strong>Tanggal:</strong> <span id="modal-ticket-date"></span></div>
                                <div><strong>User:</strong> <span id="modal-ticket-user"></span></div>
                                <div><strong>Jumlah Item:</strong> <span id="modal-ticket-count"></span></div>
                            </div>
        ...
        <!-- Modal Approval -->
        <div x-data="{ showStatusDetail: false, status: {}, role: 'user' }" x-show="showStatusDetail" x-cloak
            class="fixed inset-0 z-50 overflow-y-auto" style="display: none;" id="status-detail-modal">
            <div class="flex items-center justify-center min-h-screen">
                ...
                <div class="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 z-10">
                    ...
                    <div class="bg-gray-50 px-6 py-4 rounded-b-lg flex justify-end">
                        <button @click="showStatusDetail = false"
                            class="btn btn-secondary bg-gray-400 hover:bg-gray-500 text-white px-4 py-2 rounded-md text-sm">
                            Tutup
                        </button>
                    </div>
        ...
        <script>
            ...
            function tambahRow() {
                ...
                if (selectKategori.value) {
                    loadItemsByKategori(selectKategori, selectNama).then(() => {
                        // Setelah item dimuat, cek apakah selectNama punya value
                        if (selectNama.value) {
                            loadTipeByKategoriAndJenis(selectKategori, selectNama, selectTipe);
                        }
                    });
            ...
            function showDetail(tiket) {
                ...
                fetch(`/requestbarang/${tiket}`)
                    ...
                    .then(data => {
                        ...
                        document.getElementById('modal-ticket-date').textContent = new Date(data.tanggal_permintaan).toLocaleDateString('id-ID', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
                    ...
                    .catch(() => {
                        modalSpinner.style.display = 'none';
                        modalContent.innerHTML = `<div class="text-center text-red-600 p-4"><i class="fas fa-exclamation-triangle text-2xl mb-2"></i><p>Detail tidak ditemukan.</p></div>`;
                        modalContent.style.display = 'block';
                    });
            }
        </script>
